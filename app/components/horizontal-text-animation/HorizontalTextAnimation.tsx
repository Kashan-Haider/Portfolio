...
const ScrollingText: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      gsap.set(wrapper.children[0], {
        x: -(position.current[0] % textWidth.current),
      });
      gsap.set(wrapper.children[1], {
        x: -(position.current[1] % textWidth.current),
      });
      gsap.set(wrapper.children[2], {
        x: -(position.current[2] % textWidth.current),
      });
    };
  ...
  return (
    <div className="max-w-[100vw] overflow-hidden  py-5">
      <div ref={containerRef} className=" text-[#464646] will-change-transform">
        <div ref={wrapperRef}>
          {[0, 1, 2].map((rowIndex) => (
            <div
              key={rowIndex}
              className="flex gap-1 md:gap-3 whitespace-nowrap mt-4"
            >
